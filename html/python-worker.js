...
let functions = {
	RUN: src => {
		...
		pyodide.runPythonAsync(exec_template).then(() => {
			send('READY')
		})
	},
	...
	DEBUG: src => {
		send('BUSY')
		self.exec_object = src
		pyodide.runPythonAsync(exec_template_debugger).then(() => {
			send('EndDEBUGGER')
			send('READY')
		})
	}
...
onmessage = data => {
	data = data.data
	functions[data.type](data.value)
}